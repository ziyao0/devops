package com.ziyao.devops.jenkins.core;

import com.ziyao.devops.jenkins.api.JenkinsHttpApi;
import com.ziyao.devops.jenkins.api.JenkinsHttpConnection;
import com.ziyao.devops.jenkins.api.util.EncodingUtils;
import com.ziyao.devops.jenkins.api.util.UrlUtils;
import com.ziyao.devops.jenkins.helper.JenkinsVersion;
import com.ziyao.devops.jenkins.model.*;
...
public class JenkinsServer implements Closeable {
    ...
    public JenkinsServer(URI serverUri) {
        this(new JenkinsHttpApi(serverUri));
    }
    ...
    public JenkinsServer(URI serverUri, String username, String passwordOrToken) {
        this(new JenkinsHttpApi(serverUri, username, passwordOrToken));
    }
    ...
    public Map<String, View> getViews(FolderJob folder) throws IOException {
        ...
        return Maps.uniqueIndex(views, new Function<View, String>() {
            ...
            public String apply(View view) {
                ...
                // : Think about the following? Does there exists a
    ...
    public View getView(FolderJob folder, String name) throws IOException {
        try {
            ...
            // : Think about the following? Does there exists a simpler/more
            // elegant method?
    ...
    public Optional<FolderJob> getFolderJob(Job job) throws IOException {
        try {
            ...
            if (e.getStatusCode() == HttpStatus.SC_NOT_FOUND) {
                // : Check if this is a good idea ? What about
                // Optional.absent() ?
                return null;
            }
            throw e;
    ...
    public Queue getQueue() throws IOException {
        // : Check if using depth=1 is a good idea?
